...
init_lba <-function()
{
  ...
  list(v=cbind(runif(stan.dat$NUM_SUBJ,1,5)),
       A=cbind(runif(stan.dat$NUM_SUBJ,1,5)),
       psi=cbind(runif(stan.dat$NUM_SUBJ,1,5)),
       v_mu = cbind(runif(stan.dat$NUM_CHOICES,5,10)))}
...
#fit_lba_drift <- lba$variational(data = stan.dat,iter=50000, algorithm="fullrank")

fit_lba_drift$save_object()

# Check Recovery of Drift Rates 
v_est <- fit_lba_drift$summary("v")[,1:2]

# Extract Simulated Driftrates 
v_subj <- matrix(NaN,nrow = n_subjects,ncol = n_choice)
for (i in 1:n_subjects)
{
  
  
  v_subj[i,] <- colMeans(LBA_dat[[i]][[1]][,23:27])


}
v_subj_sim <- as_tibble(cbind(seq(1:n_subjects),v_subj))
colnames(v_subj_sim) <- c("ID","v1_sim","v2_sim","v3_sim","v4_sim","v5_sim")

v_subj_est <- mean_v_est %>% separate(variable,into=c("ID","Choice"),sep = ",")  %>%
  mutate(ID= str_remove_all(ID,pattern="v\\["), Choice=str_remove_all(Choice,"\\]")) %>%
  pivot_wider(., names_from = Choice, values_from = mean) %>%
  rename("v1_est"=`1`,"v2_est" = `2`,"v3_est" = `3`,"v4_est" = `4`,"v5_est" = `5`) %>% mutate(ID = as.double(ID))


v_data <- left_join(v_subj_est, v_subj_sim, by="ID")

cor(v_data$v1_est,v_data$v1_sim)
cor(v_data$v2_est,v_data$v2_sim)
cor(v_data$v3_est,v_data$v3_sim)
cor(v_data$v4_est,v_data$v4_sim)
cor(v_data$v5_est,v_data$v5_sim)
